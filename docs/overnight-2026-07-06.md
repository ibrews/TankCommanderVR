...
## IMPORTANT: agent worktree staleness bug

5 of the first 8 background agents' isolated worktrees were branched from `4f19e66` — **51 commits behind** master at launch time (missing 623 lines of `main.gd` changes and 215 lines of `xr_rig.gd` changes: plane-spawn fixes, avatar visibility fixes, pause-menu fixes, glove-rig fixes, etc., commits `ccfdb24`..`f11046e`). The other 3 landed on `eb6dc36` (correct, current at launch time). Root cause unclear (looks like a race in the isolation:worktree mechanism), but the practical fix is: **never plain-merge a stale-based agent branch — always `git rebase --onto master <stale-base> <branch>` first**, resolve any conflicts by hand, then merge. Did this successfully for the splash-screen branch (one conflict in `main.gd`'s `_ready()`, resolved by keeping both the EventLog/game-over wiring from master and the splash-gating logic from the branch). Will repeat for every other stale-based agent as it completes: task 2 (MP crash, opus), task 5 (Spider-Man, opus), task 6/7/8 (wifi/coffee/energy-drink polish, sonnet), task 17 (weapons, fable).
