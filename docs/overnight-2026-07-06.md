...
- **v0.6.18 (code 28) is live on App Lab alpha**, build `3312581042280632`, uploaded 2026-07-06 ~16:39 UTC. Alpha invite link (unchanged): `https://www.meta.com/s/6mSdrdXX6`.
- **v0.6.19 (code 29)** — versus multi-vehicle support (tank/jeep/boat/plane, see "Follow-up: versus multi-vehicle" below) + a security scrub of a leaked Cloudflare token. Built and committed; **App Lab upload for this one is pending an on-device sanity check** — the connected Quest disconnected mid-session and reconnected as `unauthorized` (needs a physical "Allow USB debugging?" tap in-headset, which I can't do remotely). Verified thoroughly on desktop instead (see below) but held off shipping without the on-device pass that every other build this session got, to keep that discipline consistent. **Say the word once the headset is reachable again and I'll finish the verify-and-ship pass** — it's a two-minute follow-up, everything else is done.

## Follow-up: security scrub + versus multi-vehicle (same day, later session)

- **Security**: `multiplayer/wrangler.toml`'s `LOG_TOKEN` was committed in plaintext on this public GitHub repo (`github.com/ibrews/TankCommanderVR`). Removed from the tracked file, redeployed the Worker without the `[vars]` binding, and rotated to a fresh value stored via `wrangler secret put` (verified against the live `/logs/list` endpoint). Old value stays in git history (not rewriting history for this repo, per Alex's call) but is no longer a valid/live credential.
- **Versus multi-vehicle support**: co-op stays tank-only (a deliberate scope decision — see the plan at the time, `C:\Users\Sam\.claude\plans\iterative-wobbling-starlight.md`), but **versus mode now supports tank, jeep, boat, and plane** instead of silently forcing tank regardless of the menu pick. Root cause: `net.gd` read `tank.turret.rotation.y`/`tank.gun_elev` directly (PlayerTank-only fields) every frame; a generic `get_aim_yaw_pitch()` accessor was added to all four vehicle classes instead. A second, independent bug found during planning (not in the original report): `main.gd` passed the wrong variable to `setup_versus()` (`player`, only ever non-null for tank, instead of the generic `vehicle`) — would have null-crashed any non-tank versus session immediately.
  - **v1 scope cut, not hidden**: jeep/boat opponent replicas show position only — their gun mesh is fused into one unified hull mesh (unlike the tank's genuinely separate turret), so it doesn't visually track aim yet. Splitting those meshes is real additional scope for a future pass.
  - **Bonus fixes found while auditing the touched code**: the co-op coax MG never actually fired for a client-gunner (a real, separate, pre-existing bug — `_update_weapons()` never ran on the client's puppet tank instance at all, so its local trigger-hold state was silently discarded) — fixed by threading `gunner_mg_held` through the existing `c_gunner` RPC/relay-state path, mirroring how aim already worked. Also factored a duplicated stick-fallback merge into a new `effective_turret_input()` (mirrors the existing `effective_drive()` pattern) so a client-gunner's thumbstick aim gets sent over the network, not just the physical grip.
  - **Verification**: caught and fixed a real GDScript compile error on the first live test (`var aim := tank.get_aim_yaw_pitch()` can't type-infer through a duck-typed call on a loosely-typed variable — needed explicit `var aim: Vector2 = ...`). After that fix, ran a genuine two-desktop-instance VERSUS connection test (host=jeep, client=tank, via a new `--vs-vehicle=<name>`/`--versus` modifier added to the existing `--mp-host`/`--mp-join` CLI test hooks) — both instances connected and ran with no new error types; the only error seen matches a pre-existing, already-documented artifact in this repo's own `mp_join.log` (asymmetric 18s/22s test-harness auto-quit timers spamming "RPC via peer not connected" after one side quits first, not a real connection bug). **Still owed: real two-headset hardware verification** — same limitation as the rest of this session's MP work.
...
- **Cloudflare Worker secret scrubbed (2026-07-06, later same day)**: `multiplayer/wrangler.toml`'s `LOG_TOKEN` was a plaintext shared secret committed to this repo, which is public on GitHub (`github.com/ibrews/TankCommanderVR`). Removed from the tracked file, redeployed the worker without the `[vars]` binding, and moved it to a proper Cloudflare secret (`wrangler secret put LOG_TOKEN`) with a freshly rotated value — worker.js reads `env.LOG_TOKEN` identically either way, no code change needed. Verified the new token against the live `/logs/list` endpoint. Old value stays in git history per Alex's call (not doing a history rewrite for a low-stakes indie repo) but is no longer live/valid. New token value is not written anywhere in this repo — Alex has it from the chat where this was done; rotate again the same way (`npx wrangler secret put LOG_TOKEN` from `multiplayer/`) if needed.
